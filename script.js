...
function displayShipUpgrades() {
  ...
  const availableUpgrades = shipNames.slice(currentShipIndex + 1);

  const shipUpgradesList = document.getElementById("shipUpgradesList");
  shipUpgradesList.innerHTML = "";

  if (availableUpgrades.length > 0) {
    availableUpgrades.forEach((shipName) => {
      const li = document.createElement("li");
      li.textContent = `${shipName} (Cost: ${ships[shipName].cost} Coins)`;
...
function handleShipUpgrade() {
  ...
  if (currentShipIndex < shipNames.length - 1) {
    ...
    if (coins >= nextShipCost) {
      coins -= nextShipCost;
      currentShip = nextShip;
      console.log(nextShip);
      console.log(currentShip);
      document.getElementById("current-ship").textContent = currentShip;
      document.getElementById("currentShipName").textContent = currentShip;
      document.getElementById("currentShipCapacity").textContent =
        ships[currentShip].capacity;

      upgradeMessage.textContent = `Successfully upgraded to a ${currentShip}!`;
      displayShipUpgrades();
      updateStatus();
    } else {
      upgradeMessage.textContent = `You don't have enough coins to upgrade to a ${nextShip}.`;
    }
  } else {
...
function buyGoods() {
  ...
  if (itemPrice !== null && coins >= totalCost) {
    previewDiv.innerHTML = `Preview: You are about to buy ${buyAmount} ${buyItem}(s) for a total of ${totalCost} coins. Click 'buy' again to confirm purchase.`;
    document.getElementById("buy-button").onclick = function () {
      if (coins >= totalCost) {
        ...
        previewDiv.innerHTML = `Not enough coins to complete this purchase.`;
      }
    };
  } else {
...
function sellGoods() {
  ...
  if (itemPrice !== null && inventory.includes(sellItem)) {
    ...
    if (itemCount >= sellAmount) {
      ...
      previewDiv.innerHTML = `Preview: You are about to sell ${sellAmount} ${sellItem}(s) for a total of ${totalSell} coins. Click 'sell' again to confirm sale.`;
...
document
  ...
  .addEventListener("input", function () {
    ...
    if (coins >= travelCost) {
      previewDiv.innerHTML = `Preview: Travel to ${destination} for ${travelCost} coins.`;
    } else {
      previewDiv.innerHTML = `Not enough coins to travel to ${destination}.`;
    }
  });
...
document
  .getElementById("ship-upgrade-tab")
...
document
  ...
  .addEventListener("click", handleShipUpgrade);
...
function buildMarketPricesTables() {
  ...
  items.forEach((item) => {
    ...
    for (const location in goodsPrices) {
      if (goodsPrices[location][item]) {
        // check if the item exists in the location
...
function travelToDestination() {
  ...
  if (coins >= travelCost) {
    coins -= travelCost;
    logAction(
      `Traveled from ${currentLocation} to ${destination} for ${travelCost} coins.`
    );
    currentLocation = destination;
    updateStatus();
    updateBuyOptions();
    updateTravelOptions();
    document.getElementById("travel-preview").innerHTML = ""; // Clear preview after successful travel
  } else {
...
function updateStatus() {
  ...
  document.getElementById(
    "current-location"
  ).innerText = `Current Location: ${currentLocation}`;
  document.getElementById("inventory").innerText = `Inventory: ${
    inventory.length ? inventory.join(", ") : "Empty"
  }`;
  document.getElementById("currentShipName").innerText = `Ship: ${currentShip}`;
  document.getElementById(
    "currentShipCapacity"
  ).innerText = `Capacity: ${ships[currentShip].capacity}`;
  ...
  if (distinctItems.length > 0) {
    inventoryDisplay += distinctItems
      .map((item) => `${item} x${inventoryCounts[item]}`)
      .join(", ");
  } else {
